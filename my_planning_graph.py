...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        return any(~a in actionA.effects for a in actionB.effects)
    ...
    def _interference(self, actionA, actionB):
        ...
        return any([~a in actionA.effects for a in actionB.preconditions]+[~a in actionB.effects for a in actionA.preconditions])
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        p_layer=self.parent_layer
        parents=self.parents
        return all(p_layer.is_mutex(a,b) for a in parents[literalA] for b in parents[literalB]) and all(p_layer.is_mutex(b, a) for a in parents[literalA] for b in parents[literalB])
...
class PlanningGraph:
    ...
    def h_maxlevel(self):
        ...
        for g in goal:
            for index,layer in enumerate(self.literal_layers):
                if g in layer:
                    level_count=max(index,level_count)
                    break
        return level_count  
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            if goals_met==True and not_mutex==True:
                return depth
            self._extend()
            depth += 1
        return -1
